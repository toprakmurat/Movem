...
def game_start() -> Any:
    ...
    # We store the selected modes in the user's session
    session['game_modes'] = request.form.getlist('game_types') or ['1', '4'] # Default if empty
    session['streak'] = 0
...
def game_play() -> str:
    ...
    return render_template(
        "game.html",
        prompt="Which movie had a higher budget?",  # Static prompt for demo
        options=options,
        streak=streak,
        result=result,
    )
